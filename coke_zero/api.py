...
from coke_zero.services.schemas.events import Signal, UIEvent
...
class CokeZeroRuntime:
    ...
    async def start(self) -> None:
        ...
        attrib_window_s = float(os.environ.get("COKE_ZERO_ATTRIB_WINDOW_S", "0.5"))
        ...
        self._tasks = [
            asyncio.create_task(coro, name=f"coke-zero-api-{name}")
            for name, coro in services
        ]
        log.info("coke-zero API runtime started with %d services", len(self._tasks))
    ...
    async def stop(self) -> None:
        ...
        self._tasks.clear()
        self.bus.close()
        log.info("coke-zero API runtime stopped")
...
runtime = CokeZeroRuntime()


@asynccontextmanager
async def lifespan(app: FastAPI):
    logging.basicConfig(
        level=os.environ.get("COKE_ZERO_LOG_LEVEL", "INFO"),
...
app = FastAPI(title="coke-zero API Bridge", lifespan=lifespan)
...
async def health() -> dict[str, Any]:
    return {"ok": True, "service": "coke-zero-api-bridge"}
