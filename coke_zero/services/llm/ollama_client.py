...
* ``COKE_ZERO_OLLAMA_URL`` — base URL of the Ollama daemon. Over Tailscale this
  is typically ``http://<machine-name>:11434`` or ``http://<100.x.x.x>:11434``.
* ``COKE_ZERO_OLLAMA_MODEL`` — the model tag to use (e.g. ``gemma4:26b``).
* ``COKE_ZERO_OLLAMA_TIMEOUT_S`` — per-request HTTP timeout (default 180 s).
"""
...
class OllamaLLMClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._kb = kb
        self._model = model or os.environ.get("COKE_ZERO_OLLAMA_MODEL", DEFAULT_OLLAMA_MODEL)
        url = base_url or os.environ.get("COKE_ZERO_OLLAMA_URL", DEFAULT_OLLAMA_URL)
        self._base_url = url.rstrip("/")
        timeout = timeout_s
        if timeout is None:
            timeout_env = os.environ.get("COKE_ZERO_OLLAMA_TIMEOUT_S")
