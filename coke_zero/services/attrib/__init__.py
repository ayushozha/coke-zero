...
from coke_zero.services.nia_context import NiaContextProvider, NiaContextResult
from coke_zero.services.schemas.events import Anomaly, Domain
...
class AttribService:
    ...
    async def _process(self, anomalies: list[Anomaly]) -> None:
        ...
        for a in anomalies:
            for sid in a.source_signal_ids:
                for entry in self._kb.by_scenario_signal_id(base_signal_id(sid)):
        ...
        nia_result = await self._retrieve_nia_context(anomalies)
        if nia_result is not None and nia_result.available and nia_result.hits:
            context = [*context, *nia_result.to_kb_entries()]
    ...
    async def _retrieve_nia_context(
        self, anomalies: list[Anomaly]
    ) -> NiaContextResult | None:
        if self._nia_context is None:
            return None
        query = _nia_attribution_query(anomalies)
        ...
        if result.available and result.hits:
            await self._tracer.emit(
                ...
                ref_id=anomalies[0].id if anomalies else None,
                **result.as_payload(),
            )
            ...
            await self._tracer.emit(
                "attrib_primary",
                "warn",
                "nia.context -> 0 indexed source hit(s); using local KB fallback",
                ref_id=anomalies[0].id if anomalies else None,
                **result.as_payload(),
            )
        elif result.error:
            await self._tracer.emit(
                "attrib_primary",
                "warn",
                "nia.context unavailable; using local KB fallback",
                ref_id=anomalies[0].id if anomalies else None,
                **result.as_payload(),
            )
        return result
...
def _nia_attribution_query(anomalies: list[Anomaly]) -> str:
    kinds = ", ".join(sorted({a.kind for a in anomalies})) or "unknown anomaly"
    signals = ", ".join(
        sorted({base_signal_id(sid) for a in anomalies for sid in a.source_signal_ids})
    )
    summaries = "; ".join(
        str((a.payload or {}).get("summary") or a.kind) for a in anomalies[:4]
    )
    return (
        "coke-zero attribution grounding for "
        f"anomaly kinds: {kinds}; source signals: {signals or 'none'}; "
        f"summaries: {summaries}. Search README, docs, KB entries, scenarios, "
        "source notes, and code for relevant context and citations."
    )
