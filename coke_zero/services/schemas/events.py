...
OperatorActionStatus = Literal["approved", "denied", "dismissed"]

OperatorSubjectKind = Literal["ui_event", "recommendation", "decision", "unknown"]
...
class Signal(_Event):
    """Canonical coke-zero Signal — matches services/bus/schemas/signal.schema.json."""
...
class Anomaly(_Event):
    """Canonical coke-zero Anomaly — matches services/bus/schemas/anomaly.schema.json."""
...
class OperatorAction(_Event):
    """Operator disposition for a recommendation or decision.

    These events make approval, denial, and dismissal backend state instead of
    browser-only session state.
    """

    status: OperatorActionStatus
    subject_kind: OperatorSubjectKind = "unknown"
    actor: str = "operator"
